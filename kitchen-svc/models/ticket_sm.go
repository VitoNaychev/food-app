...
var ticketDeltas = []sm.Delta{
	...
	{Current: sm.State(CREATED), Event: sm.Event(START_PREPARING), Next: sm.State(PREPARING), Predicate: nil, Callback: nil},
	{Current: sm.State(PREPARING), Event: sm.Event(FINISH_PREPARING), Next: sm.State(READY_FOR_PICKUP), Predicate: nil, Callback: nil},
